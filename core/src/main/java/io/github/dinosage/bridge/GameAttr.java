package io.github.dinosage.bridge;

public class GameAttr {

    // Plank Attributes
    public static int START_PLANK_COUNT = 100;
    public static float PLANK_SPAWN_INTERVAL = 2f;
    public static final float PLANK_ITEM_VEL_X = -5f;
    public static final int PLANK_POWER_BOOST = 20;


    public static float PLANK_WIDTH = 3f;
    public static float PLANK_HEIGHT = 0.5f;
    public static float PLANK_VEL_X = -10f;

    // Misc Attributes
    public static float SHAPE_BORDER = 0.1f;
    public static float OOB_BUFFER = 5f; // out of bounds buffer
    public static boolean DEBUG_UI = true;

    public static float OBSTACLE_SPAWN_INTERVAL = 5f;
    public static float OBSTACLE_VEL_X = -5f;
}
